    ...
    <div class="login-box">
        ...
        <div class="box">
            ...
            <div class="b-tips">
                ...
                <div>
                    <div class="b-title">JIMCLOUD服务器</div>
                    <div class="card">注册即可享受最低折扣优惠及大量优惠券</div>
                    <ul>
                        <li class="normal">低至五折代金券</li>
                        <li class="normal">全流程安全防护</li>
                        <li class="normal">各大应用无缝对接</li>
                        <li class="normal">全天24小时售后服务</li>
                    </ul>
                </div>
            </div>
            ...
            <el-card class="login-main">
                <!-- 标题 -->
                <div class="title second">JIMCLOUD服务器</div>
                <div class="goLogin">
                    <el-button @click="gotoView('/login')" link>已有账号，去登陆 ></el-button>
                </div>
                    ...
                    <el-tab-pane name="first">
                        ...
                        <el-form :model="userForm" :rules="rules" ref="ruleFormRef">
                            <el-row :gutter="20">
                                <el-col :span="12">
                                    <el-form-item>
                                        <el-input size="large" class="noborder" placeholder="请输入姓氏"
                                            v-model="userForm.firstname" />
                                    </el-form-item>
                                </el-col>
                                <el-col :span="12">
                                    <el-form-item>
                                        <el-input size="large" class="noborder" placeholder="请输入姓名"
                                            v-model="userForm.lastname" />
                                    </el-form-item>
                                </el-col>
                            </el-row>
                            <el-form-item prop="email">
                                <el-input size="large" class="noborder" placeholder="请输入邮箱" v-model="userForm.email" />
                            </el-form-item>
                            <el-form-item prop="phone">
                                <el-input size="large" class="noborder" placeholder="请输入手机号" v-model="userForm.phone" />
                            </el-form-item>
                            <el-form-item prop="password">
                                <el-input size="large" type="password" show-password placeholder="密码"
                                    v-model="userForm.password" />
                            </el-form-item>
                            <el-form-item prop="confirmPw">
                                <el-input size="large" type="password" show-password placeholder="确认密码"
                                    v-model="userForm.confirmPw" />
                            </el-form-item>
                            <el-form-item>
                                <el-select v-model="userForm.sales" size="large" :teleported="false" placeholder="选择你的商务代表">
                                    <el-option v-for="item in sales" :key="item" :label="item" :value="item" />
                                </el-select>
                            </el-form-item>
                            <div class="btn-submit">
                                <el-button class="submit-btn" @click="submitForm(ruleFormRef)" type="primary"
                                    size="large">注册</el-button>
                            </div>
                            <el-form-item>
                                <el-checkbox v-model="userForm.isRead">我已阅读并同意订阅<el-button
                                        @click="serverdialogVisible = true" type="primary"
                                        link>JimCloud服务协议</el-button>和<el-button style="margin:0"
                                        @click="privacydialogVisible = true" type="primary"
                                        link>JimCloud隐私条款</el-button></el-checkbox>
                            </el-form-item>
                        </el-form>
                    </el-tab-pane>
                </el-tabs>
...
let sales = ref(null)
onMounted(async () => {
    sales.value = await userStore.ClientSales()
})
...
const rules = reactive<FormRules>({
    email: [
        ...
        { required: true, message: '邮箱格式不正确！', trigger: 'blur', pattern: regs.email },
    ],
    phone: [
        { required: true, message: '手机号不能为空！', trigger: 'blur' },
        { required: true, message: '手机号格式不正确！', trigger: 'blur', pattern: regs.phone }
    ],
    password: [
...
const submitForm = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate(async (valid, fields) => {
        if (valid) {
            ...
            if (userForm.password != userForm.confirmPw) {
                ElMessage({
                    message: '密码不一致',
                    type: 'error',
                })
            ...
            try {
                const res = await userStore.ClientRegister(userForm.firstname, userForm.lastname, userForm.email, userForm.phone, userForm.password,userForm.sales)
                ElMessage({
                    message: res,
                    type: 'success',
                })
